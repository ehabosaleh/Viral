...
static void migrate(sg4::Host *src, Migrant_Work* residue){
	std::vector <double> computing_power={};
	std::vector<double> RAM={};
	for (auto host_name:netzone[src->get_cname()]){
		sg4::Host *host=sg4::Host::by_name(host_name);
		computing_power.push_back(host->get_available_speed());
		RAM.push_back(stod(host->get_property("RAM")));
	}

	int destination_index=std::min_element(computing_power.begin(),computing_power.end())-computing_power.begin();
	sg4::Host * dst=sg4::Host::by_name(netzone[src->get_cname()][destination_index]);
	sg4::Mailbox *mail_box=sg4::Mailbox::by_name(src->get_name()+"_"+dst->get_name());
	mail_box->put(residue, residue->size);
	//search for a destination host;
	//send the result packet to the destination;
	//
}
...
static void local_worker(double work,Result* result_packet){
	...
	*busy=false;
	...
	work=work-new_work;
	if(work<new_work){sg4::this_actor::execute(work);supervisor->kill(); result_packet->completion=true;result_packet->residue=-1;break;}
	if (*busy==true){
...
static void work_distributor(double work,bool root,string candinate_name){
	...
	for (auto host_name:netzone[source_name])
	{
		total_computing_power+=computing_tree[source_name][host_name][0];
		total_RAM_size+=computing_tree[source_name][host_name][1];
		computing_power.push_back(computing_tree[source_name][host_name][0]);
		RAM_size.push_back(computing_tree[source_name][host_name][1]);
	}
	...
	for(auto host:netzone[source_name]){			// 1)create new actors+ create new mail boxes named as new workers
		work_per_host=work_per_FLOP*computing_tree[source_name][host][0];
